...
sources](https://github.com/brronsuite/lnd/blob/master/lnrpc/lightning.proto).
...
let lightning = new lnrpc.Lightning('localhost:10005', credentials);
```
...
of which is at the default `localhost:10005`, with an open channel between the
two nodes.
...
let lnrpcDescriptor = grpc.loadPackageDefinition(packageDefinition);
let lnrpc = lnrpcDescriptor.lnrpc;
let client = new lnrpc.Lightning('some.address:10005', credentials);
