...
5. Copy the lnd lightning.proto file (you'll find this at
  [lnrpc/lightning.proto](https://github.com/brronsuite/lnd/blob/master/lnrpc/lightning.proto))
  or just download it
    ```shell
    lnd ⛰  curl -o lightning.proto -s https://raw.githubusercontent.com/brronsuite/lnd/master/lnrpc/lightning.proto
    ```
...
```shell
lnd ⛰  curl -o router.proto -s https://raw.githubusercontent.com/brronsuite/lnd/master/lnrpc/routerrpc/router.proto
...
channel = grpc.secure_channel('localhost:10005', creds)
...
is at the default `localhost:10005`, with an open channel between the two nodes.
...
Now, create an invoice for your node at `localhost:10005`and send a payment to
...
channel = grpc.secure_channel('localhost:10005', combined_creds)
stub = lnrpc.LightningStub(channel)
