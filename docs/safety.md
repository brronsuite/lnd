...
By default the two API connections `lnd` offers (gRPC on port 10005 and REST on
...
There is
[a tool being worked on](https://github.com/brronsuite/lnd/pull/2373)
...
* If an external IP is set (either with `--externalip` or `--tlsextraip`) these
  ...
  point 1.
* If port `9731` (or `10005` for gRPC) was forwarded on the router, these
